...
switch (Misc::get_post_action('incluir', 'excluir'))
{
	case 'incluir':
		...
		{
			$return = 'return='.urlencode('Para a inclusão é necessário selecionar um roteiro e um momento.');
		}
		break;
	case 'excluir':
		...
		{
			$return = 'return='.urlencode('Para a exclusão é necessário selecionar um roteiro.');
		}
		break;
}
$sql->close();
header ('Location: e_sistema.php?sistema2='.$sistema.'&'.$return.'&sistema1='.$sistema_filtro.'&momento='.$momento_filtro.'');
?>
