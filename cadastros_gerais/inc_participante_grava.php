...
$participante = new Participantes(null);
$participante->NU_CONTRATO = $_POST['NU_CONTRATO'];
$participante->TIPO_PARTICIPANTE = $_POST['TIPO_PARTICIPANTE'];
$participante->NOME_PARTICIPANTE = $_POST['NOME_PARTICIPANTE'];
$participante->DT_NASCIMENTO = @date('Y-m-d', @strtotime(str_replace("/", "-", $_POST['DT_NASCIMENTO'])));
...
if ($participante->salvar())
{
   ...
   echo "<script>openBox('participante_return.php?height=200&width=380&modal=true&destino=".urlencode("inc_participante.php")."&return=".urlencode($participante->errorstr)."&NU_CONTRATO=".$_POST['NU_CONTRATO']."&TIPO_PARTICIPANTE=".urlencode($_POST['TIPO_PARTICIPANTE'])."&NOME_PARTICIPANTE=".urlencode($_POST['NOME_PARTICIPANTE'])."&DT_NASCIMENTO=".urlencode($_POST['DT_NASCIMENTO'])."&origem_destino=".urlencode($_POST['origem_destino'])."&endereco=".urlencode($_POST['endereco'])."&numero=".urlencode($_POST['numero'])."&complemento=".urlencode($_POST['complemento'])."&bairro=".urlencode($_POST['bairro'])."&cidade=".urlencode($_POST['cidade'])."&estado=".urlencode($_POST['estado'])."&telefone=".urlencode($_POST['telefone'])."&cep=".urlencode($_POST['cep'])."&cpf_cnpj=".urlencode($_POST['cpf_cnpj'])."&email=".urlencode($_POST['email'])."&renda=".urlencode($_POST['renda'])."');</script>";
?>
