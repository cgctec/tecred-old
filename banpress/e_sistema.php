...
<input type="hidden" name="momento2" value="<?php echo $_GET['momento2'];?>"/>
<input type="hidden" name="momento" value="<?php echo $_GET['momento2'];?>"/>
<input type="hidden" name="momento_filtro" value="<?php echo $_GET['momento'];?>"/>
<center><div style="position:relative; top: -2px; z-index:1;"><a href="e_sistema_form.php"><img id="voltar" src="../imagens/bt-voltar-over.png"></a></div></center>
...
<table>
	<tr>
		<td><input style="padding-right:50px;" type="image" name="excluir" value="excluir" src="../imagens/up.png"/></td>
...
$result = mysql_query('SELECT momentos.nome as momento, roteiro_sistema_momento.cod_momento as cod_momento, roteiro_sistema_momento.cod_roteiro as cod_roteiro FROM roteiro_sistema_momento INNER JOIN roteiro ON roteiro.uid = roteiro_sistema_momento.cod_roteiro INNER JOIN momentos ON momentos.codigo = roteiro_sistema_momento.cod_momento INNER JOIN tabela_produtos ON tabela_produtos.NU_PRODUTO = roteiro_sistema_momento.cod_sistema WHERE roteiro_sistema_momento.cod_sistema = '.$_GET['sistema2'].' '.$condição.';',$con);
$sql->close();
